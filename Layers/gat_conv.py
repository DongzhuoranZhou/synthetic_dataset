...
class GATConv(MessagePassing):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.num_feats = kwargs.get('num_feats', None)
        self.with_ACM = kwargs.get('with_ACM', False)
        if self.with_ACM:
            self.out_channels = out_channels = self.num_feats
        # self.w_norm = kwargs.get('w_norm', None)
    ...
    def forward(self, x: Union[Tensor, OptPairTensor], edge_index: Adj,
                ...
                return_attention_weights=None,w_for_norm=None,**kwargs):
        ...
        if w_for_norm is not None:
            if isinstance(x, Tensor):
                assert x.dim() == 2, "Static graphs not supported in 'GATConv'"
                x_src = x_dst = x.view(-1, H, self.num_feats)
